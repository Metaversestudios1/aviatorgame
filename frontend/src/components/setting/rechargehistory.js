...
const Payment = () => {
  ...
  const fetchPayment = async () => {
    ...
    if (id) {
      ...
      apiUrl = `${process.env.REACT_APP_BACKEND_URL}/api/getpaymentid?id=${id}&transactionType=recharge`;
    } else {
      // If no ID in URL, fetch all users as usual
      apiUrl = `${process.env.REACT_APP_BACKEND_URL}/api/getpayment?page=${page}&limit=${pageSize}&search=${search}&transactionType=recharge`;
    }
      // const res = await fetch(
      //   `${process.env.REACT_APP_BACKEND_URL}/api/getpayment?page=${page}&limit=${pageSize}&search=${search}&transactionType=recharge`
      // );
      ...
      try {
        const res = await fetch(
          `${process.env.REACT_APP_BACKEND_URL}/api/updatetransaction/${id}`,
          {
            ...
            body: JSON.stringify({ status: newStatus,type:'recharge' }),
          }
        );
        const response = await res.json();
        if (response.success) {
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center">
        <div className="text-2xl font-bold mx-2 my-8 px-4">Recharge History</div>
      </div>
      ...
      <div className="relative overflow-x-auto m-5 mb-0">
        ...
        {noData && (
          <div className="text-center text-2xl font-semibold text-gray-500 p-10">
            No Data Found in Recharge History
          </div>
        )}
      </div>
      ...
      <div className="flex justify-end">
        ...
        <button
          ...
          className={`bg-blue-800 text-white p-2 m-5 text-sm rounded-lg ${payment.length === 0 ? "opacity-50" : ""
            }`}
        >
          Next
        </button>
